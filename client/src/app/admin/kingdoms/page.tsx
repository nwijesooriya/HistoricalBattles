...
export default function AdminKingdomsPage() {
  ...
  const fetchKingdoms = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/kingdoms`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingKingdom
        ? `${API_BASE_URL}/kingdoms/${editingKingdom._id}`
  ...
  const handleEdit = (kingdom: Kingdom) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this kingdom?')) return;

    const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/v1';
    const token = localStorage.getItem('admin_token');

    try {
      const res = await fetch(`${API_BASE_URL}/kingdoms/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          ...(token && { Authorization: `Bearer ${token}` }),
      ...
      if (!res.ok) throw new Error('Failed to delete kingdom');

      fetchKingdoms();
    } catch (error) {
      console.error('Failed to delete kingdom:', error);
      alert('Failed to delete kingdom');
    }
  };
              ...
              {kingdoms.map((kingdom) => (
                <tr key={kingdom._id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <button onClick={() => handleDelete(kingdom._id)} className="text-red-600 hover:text-red-900">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
